...
import com.flyco.tablayout.listener.TabClickListener;
import com.flyco.tablayout.utils.SharedPreferenceUtils;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextUnSelectedSize = ta.getDimension(com.flyco.tablayout.R.styleable.SlidingTabLayout_tl_unselected_textsize, sp2px(14));
...
//        updateTabStyles();
        updateTabStylesWithPosition(mTabCount - 1);
    }

    public void addNewTab(int position, String title) {
        View tabView = View.inflate(mContext, R.layout.layout_tab, null);
        if (mTitles != null) {
            mTitles.add(position, title);
        }
        CharSequence pageTitle = mTitles == null ? "" : mTitles.get(position);
        addTab(position, pageTitle.toString(), tabView);
        this.mTabCount = mTitles == null ? mViewPager.getAdapter().getCount() : mTitles.size();
        updateTabStylesWithPosition(position);
    }

    /**
     * 创建并添加tab
     */
    private void addTab(final int position, String title, final View tabView) {
        ...
        tabView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (position != -1) {
                    if (null != tabClickListener && position == checkPosition) {
                        long vipLeftTime = (Long) SharedPreferenceUtils.get("pref_end_vip_time_mills", 0L) - System.currentTimeMillis();
                        if (vipLeftTime <= 0) {
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            if (tv_tab_title != null) {
                ...
                tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, i == mCurrentTab ? mTextsize : mTextUnSelectedSize);
    ...
    private void updateTabStylesWithPosition(int position) {
        ...
        if (tv_tab_title != null) {
            ...
            tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, position == mCurrentTab ? mTextsize : mTextUnSelectedSize);
            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
            if (mTextAllCaps) {
                tv_tab_title.setText(tv_tab_title.getText().toString().toUpperCase());
            }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            if (tab_title != null) {
                ...
                tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, isSelect ? mTextsize : mTextUnSelectedSize);
